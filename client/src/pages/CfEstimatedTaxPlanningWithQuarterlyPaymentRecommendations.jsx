...
// Feature: Estimated tax planning with quarterly payment recommendations
...
export default function CfEstimatedTaxPlanningWithQuarterlyPaymentRecommendations() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-estimated-tax-planning-with-quarterly-payment-recommendations/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-estimated-tax-planning-with-quarterly-payment-recommendations', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-estimated-tax-planning-with-quarterly-payment-recommendations/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Estimated tax planning with quarterly payment recommendations</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AiTaxPrep</p>
      </div>
