...
export default function CfMultiStateTaxPlanningForStateSpecific() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-state-tax-planning-for-state-specific-deductions-and-credits/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-state-tax-planning-for-state-specific-deductions-and-credits', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-state-tax-planning-for-state-specific-deductions-and-credits/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-state tax planning for state-specific deductions and credits</h1>
