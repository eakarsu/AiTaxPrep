...
export default function CfDocumentAutoCategorizationViaReceiptOcrMl() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-document-auto-categorization-via-receipt-ocr-ml/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-document-auto-categorization-via-receipt-ocr-ml', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-document-auto-categorization-via-receipt-ocr-ml/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Document auto-categorization via receipt OCR + ML</h1>
