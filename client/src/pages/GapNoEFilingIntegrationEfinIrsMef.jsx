...
export default function GapNoEFilingIntegrationEfinIrsMef() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-e-filing-integration-efin-irs-mef/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-e-filing-integration-efin-irs-mef', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-e-filing-integration-efin-irs-mef/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No e-filing integration (EFIN, IRS MeF)</h1>
