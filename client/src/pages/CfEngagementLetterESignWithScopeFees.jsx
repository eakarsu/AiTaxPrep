...
// Feature: Engagement letter e-sign with scope/fees
...
export default function CfEngagementLetterESignWithScopeFees() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-engagement-letter-e-sign-with-scope-fees/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-engagement-letter-e-sign-with-scope-fees', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-engagement-letter-e-sign-with-scope-fees/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Engagement letter e-sign with scope/fees</h1>
