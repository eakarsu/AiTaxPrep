...
export default function GapNoAutomatedAuditRiskEarlyWarningMonitor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-automated-audit-risk-early-warning-monitor/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-automated-audit-risk-early-warning-monitor', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-automated-audit-risk-early-warning-monitor/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No automated audit-risk early-warning monitor</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AiTaxPrep</p>
      </div>
