...
export default function CfIrsNoticeCp1099LetterAutoResponse() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-irs-notice-cp-1099-letter-auto-response-drafter/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-irs-notice-cp-1099-letter-auto-response-drafter', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-irs-notice-cp-1099-letter-auto-response-drafter/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">IRS notice (CP/1099-Letter) auto-response drafter</h1>
