...
export default function GapLimitedIntegrationsModuleExistsButNotDeeply() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-integrations-module-exists-but-not-deeply-wired/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-limited-integrations-module-exists-but-not-deeply-wired', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-integrations-module-exists-but-not-deeply-wired/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Limited integrations (module exists but not deeply wired)</h1>
