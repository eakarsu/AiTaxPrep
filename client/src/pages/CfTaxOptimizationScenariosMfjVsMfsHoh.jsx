...
// Feature: Tax optimization scenarios (MFJ vs MFS, HOH) with outcome comparison
...
export default function CfTaxOptimizationScenariosMfjVsMfsHoh() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-tax-optimization-scenarios-mfj-vs-mfs-hoh-with/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-tax-optimization-scenarios-mfj-vs-mfs-hoh-with', project: 'AiTaxPrep' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-tax-optimization-scenarios-mfj-vs-mfs-hoh-with/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Tax optimization scenarios (MFJ vs MFS, HOH) with outcome comparison</h1>
