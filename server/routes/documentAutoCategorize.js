...
// Document auto-categorization via receipt OCR + ML
// Feature: document-auto-categorize
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Document auto-categorization via receipt OCR + ML.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'document-auto-categorize', payload, result);
    res.json({ success: true, feature: 'document-auto-categorize', ...result });
  } catch (err) {
    console.error('document-auto-categorize analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Document auto-categorization via receipt OCR + ML. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'document-auto-categorize', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'document-auto-categorize', title: 'Document auto-categorization via receipt OCR + ML', project: 'AiTaxPrep' });
});
