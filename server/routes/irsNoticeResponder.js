...
// IRS notice (CP/1099-Letter) auto-response drafter
// Feature: irs-notice-responder
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: IRS notice (CP/1099-Letter) auto-response drafter.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'irs-notice-responder', payload, result);
    res.json({ success: true, feature: 'irs-notice-responder', ...result });
  } catch (err) {
    console.error('irs-notice-responder analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: IRS notice (CP/1099-Letter) auto-response drafter. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'irs-notice-responder', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'irs-notice-responder', title: 'IRS notice (CP/1099-Letter) auto-response drafter', project: 'AiTaxPrep' });
});
