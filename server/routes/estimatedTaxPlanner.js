...
// Estimated tax planning with quarterly payment recommendations
// Feature: estimated-tax-planner
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'estimated-tax-planner', payload, result);
    res.json({ success: true, feature: 'estimated-tax-planner', ...result });
  } catch (err) {
    console.error('estimated-tax-planner analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Estimated tax planning with quarterly payment recommendations. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'estimated-tax-planner', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'estimated-tax-planner', title: 'Estimated tax planning with quarterly payment recommendations', project: 'AiTaxPrep' });
});
