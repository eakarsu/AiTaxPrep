...
// Get all deductions for a tax year (with pagination, search, sort, filter)
router.get('/tax-year/:taxYearId', validatePagination, handleValidationErrors, async (req, res) => {
  try {
    const { page = 1, limit = 50, search, sortBy = 'amount', sortOrder = 'desc', category, isItemized } = req.query;
    ...
    if (search) {
      whereClause += ` AND (d.category ILIKE $${paramIdx} OR d.description ILIKE $${paramIdx})`;
      params.push(`%${search}%`);
      paramIdx++;
    }

    if (category) {
      whereClause += ` AND d.category = $${paramIdx}`;
      params.push(category);
      paramIdx++;
    }

    if (isItemized !== undefined) {
      whereClause += ` AND d.is_itemized = $${paramIdx}`;
      params.push(isItemized === 'true');
      paramIdx++;
    }

    const allowedSorts = { amount: 'd.amount', category: 'd.category', created_at: 'd.created_at' };
    const orderBy = allowedSorts[sortBy] || 'd.amount';
    const order = sortOrder === 'asc' ? 'ASC' : 'DESC';

    const countResult = await db.query(
      `SELECT COUNT(*) as total FROM deductions d JOIN tax_years ty ON d.tax_year_id = ty.id WHERE ${whereClause}`,
      params
    );
...
router.get('/tax-year/:taxYearId/export/csv', exportLimiter, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT d.* FROM deductions d JOIN tax_years ty ON d.tax_year_id = ty.id
       WHERE d.tax_year_id = $1 AND ty.user_id = $2 ORDER BY d.amount DESC`,
      [req.params.taxYearId, req.user.id]
    );

    const headers = ['ID', 'Category', 'Description', 'Amount', 'Itemized', 'Created'];
    const rows = result.rows.map(r => [
      r.id, `"${r.category}"`, `"${r.description || ''}"`, r.amount,
      r.is_itemized ? 'Yes' : 'No', new Date(r.created_at).toLocaleDateString()
    ]);

    const csv = [headers.join(','), ...rows.map(r => r.join(','))].join('\n');
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename=deductions.csv');
    res.send(csv);
  } catch (error) {
...
router.delete('/bulk', bulkLimiter, validateBulkOperation, handleValidationErrors, async (req, res) => {
  try {
    const { ids } = req.body;
    const result = await db.query(
      'DELETE FROM deductions WHERE id = ANY($1) AND user_id = $2 RETURNING id',
      [ids, req.user.id]
    );

    await db.query(
      `INSERT INTO bulk_operations_log (user_id, operation_type, entity_type, entity_ids, details)
       VALUES ($1, 'delete', 'deductions', $2, $3)`,
      [req.user.id, JSON.stringify(ids), JSON.stringify({ deletedCount: result.rows.length })]
    );

    res.json({ message: `${result.rows.length} deduction(s) deleted`, deletedIds: result.rows.map(r => r.id) });
  } catch (error) {
    ...
    for (const id of ids) {
      const result = await db.query(
        `UPDATE deductions SET
           ...
           is_itemized = COALESCE($2, is_itemized),
           updated_at = NOW()
         WHERE id = $3 AND user_id = $4
         RETURNING id`,
        [updates.category || null, updates.isItemized !== undefined ? updates.isItemized : null, id, req.user.id]
      );
      if (result.rows.length > 0) updatedCount++;
    }

    res.json({ message: `${updatedCount} deduction(s) updated` });
  } catch (error) {
...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      amount: parseFloat(row.amount) || 0, isItemized: row.is_itemized
    });
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await db.query(
      `UPDATE deductions SET
         category = COALESCE($1, category), description = COALESCE($2, description),
         amount = COALESCE($3, amount), is_itemized = COALESCE($4, is_itemized),
         receipt_path = COALESCE($5, receipt_path), updated_at = NOW()
...
router.get('/tax-year/:taxYearId/summary', async (req, res) => {
  try {
    const result = await db.query(
      `SELECT category, COUNT(*) as count, SUM(amount) as total, BOOL_OR(is_itemized) as has_itemized
       FROM deductions d JOIN tax_years ty ON d.tax_year_id = ty.id
       WHERE d.tax_year_id = $1 AND ty.user_id = $2 GROUP BY category ORDER BY total DESC`,
      [req.params.taxYearId, req.user.id]
    );
    ...
    const standardDeductions = { 'single': 14600, 'married_filing_jointly': 29200, 'married_filing_separately': 14600, 'head_of_household': 21900, 'qualifying_widow': 29200 };
    const totalItemized = result.rows.reduce((sum, row) => sum + (parseFloat(row.total) || 0), 0);

    res.json({
      categories: result.rows.map(row => ({
        category: row.category, count: parseInt(row.count),
        total: parseFloat(row.total) || 0, hasItemized: row.has_itemized
      })),
      totalItemized,
