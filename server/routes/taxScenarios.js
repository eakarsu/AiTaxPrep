...
// Tax optimization scenarios (MFJ vs MFS, HOH) with outcome comparison
// Feature: tax-scenarios
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'tax-scenarios', payload, result);
    res.json({ success: true, feature: 'tax-scenarios', ...result });
  } catch (err) {
    console.error('tax-scenarios analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Tax optimization scenarios (MFJ vs MFS, HOH) with outcome comparison. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'tax-scenarios', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'tax-scenarios', title: 'Tax optimization scenarios (MFJ vs MFS, HOH) with outcome comparison', project: 'AiTaxPrep' });
});
