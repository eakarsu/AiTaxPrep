...
// Engagement letter e-sign with scope/fees
// Feature: engagement-esign
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'engagement-esign', payload, result);
    res.json({ success: true, feature: 'engagement-esign', ...result });
  } catch (err) {
    console.error('engagement-esign analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Engagement letter e-sign with scope/fees. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'engagement-esign', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'engagement-esign', title: 'Engagement letter e-sign with scope/fees', project: 'AiTaxPrep' });
});
